...
export default function Sidebar({ categories, isOpen, width, onWidthChange }: SidebarProps) {
      ...
      <div key={category.id}>
        <div className="flex">
          ...
          <Button
            ...
            className={`flex-1 justify-start gap-2 h-auto py-2 px-2 ${level > 0 ? `ml-${level * 4}` : ""} ${!hasChildren ? "ml-8" : ""}`}
            asChild
          >
    ...
    <>
      ...
      >
        <div className="p-4">
          ...
          <Button
            variant={location.pathname === "/blog" || location.pathname === "/" ? "secondary" : "ghost"}
            className="w-full justify-start mb-2"
            asChild
          >
            <Link to="/blog">전체 글</Link>
          </Button>
        </div>
